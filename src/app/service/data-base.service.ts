...
export class DataBaseService {
  ...
  constructor(
    ...
    private _userIdentityService: IdentitySetService,
    ...
    for (let fileNum = 0; fileNum < indices.length; fileNum++) {
      ...
      try {
        file = await JSON.parse(
          this._userCredentilasService.getData(indices[fileNum])
        );
        ...
        user = this._userIdentityService.extractDetails(xml, user);
  ...
  getIndexAndUri(): Promise<Array<file>> {
    ...
    return new Promise(async (resolve) => {
      ...
      for (let fileNum = 0; fileNum < localStorage.length; fileNum++) {
        ...
        try {
          file = await JSON.parse(
            this._userCredentilasService.getData(fileNum)!
          );
