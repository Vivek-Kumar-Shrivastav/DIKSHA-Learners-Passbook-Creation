  ...
  storeFile(token: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      for (let fileNum = 0; fileNum < this.files.length; fileNum++) {
        ...
        this.http.post<any>(urlXml, body, { responseType: 'json' }).subscribe({
          next: (res) => {
            ...
            if (noXml === null || noXml === undefined) {
              ...
              this._userCredentilasService.updateData(
                fileNum,
                JSON.stringify(data)
              );
              // this.studentData.push(this._userCredentilasService.getData(fileNum));
              // console.log(this._userCredentilasService.getData(fileNum));
            }
